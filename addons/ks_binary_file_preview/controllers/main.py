...
class KsFilePreview(http.Controller):
    ...
    def get_record_data(self, res_id=False, model=False, size=False, res_field=False, **kw):
        ...
        if res_id and model and res_field and size:
            ...
            if model == 'crm.lead'  and res_field == 'open_solar_proposal_pdf':
                lead = request.env['crm.lead'].sudo().browse(res_id)
                res_id = lead.lead_solution_id.id
                model = 'lead.solution'
                res_field = 'open_solar_proposal_pdf'
                query = """select id from ir_attachment 
                                where res_model=%s and 
                                res_id=%s and
                                res_field=%s"""
                request.env.cr.execute(query, (model, res_id, res_field))
            elif model == 'project.project' and res_field == 'open_solar_proposal_pdf':
                project = request.env['project.project'].sudo().browse(res_id)
                res_id = project.solution_id.id
                model = 'lead.solution'
                res_field = 'open_solar_proposal_pdf'
                query = """select id from ir_attachment 
                                where res_model=%s and 
